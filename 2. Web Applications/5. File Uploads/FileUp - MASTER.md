...
Capture in Burp and modify the content to include a webshell:
![[Pasted image 20240628114633.png]]
Two important parts of a file upload are `filename` and the `Content-Type`. 

### Disabling Front-end Validation
Inspect and edit:
![[Pasted image 20240628115017.png]]

Go into Page Inspector and modify the "`accept`" argument to include `.php`. Or completely remove the `checkFile()` function.
...
>[!Tip:]
>In Windows Servers, file names are case insensitive, so we may try uploading a `php` with a mixed-case (e.g. `pHp`), which may bypass the blacklist as well, and should still execute as a PHP script. Make a simple list via scripting or for all upper/lower use Burp > Payload Type > Case Modification > To upper + To lower.
## Whitelist Filters
